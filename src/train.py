...
# Import combined-tweets.txt as tab-delimited csv
...
fig = plt.figure(figsize=(8,6))
combined_tweets.groupby('subject').tweet.count().plot.bar(ylim=0)
plt.show()
...
features3 = tfidf.fit_transform(combined_tweets.tweet).toarray()
...
string = "Don't vote for Bernie, hes a communist"
...
print(clf.predict(tfidf.transform([string])))
...
print(clf2.predict(tfidf.transform([string])))
